...
class Build(build):
    def run(self):
        if path.isdir("queries"):
            dest = path.join(self.build_lib, "tree_sitter_kalei", "queries")
...
setup(
    ...
    package_data={
        "tree_sitter_kalei": ["*.pyi", "py.typed"],
        "tree_sitter_kalei.queries": ["*.scm"],
    },
    ext_package="tree_sitter_kalei",
    ext_modules=[
        Extension(
            name="_binding",
            sources=[
                "bindings/python/tree_sitter_kalei/binding.c",
                "src/parser.c",
            ],
            define_macros=[
